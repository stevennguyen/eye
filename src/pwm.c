...
shutdown(void)
{
    ...
    for (i = 0; i < DMA_CHANNELS; i++) {
        if (channels[i].dma_reg && channels[i].virtbase) {
            log_debug((char *)"shutting down dma channel %d\n", i);
            clear_channel(i);
...
setup_sighandlers(void)
{
    ...
    for (i = 0; i < 64; i++) {
        ...
        sa.sa_handler = (__sighandler_t) terminate;
        sigaction(i, &sa, NULL);
    }
}
...
map_peripheral(uint32_t base, uint32_t len)
{
    ...
    if (fd < 0) {
        fatal((char *)"rpio-pwm: Failed to open /dev/mem: %m\n");
        return NULL;
    }
    vaddr = mmap(NULL, len, PROT_READ|PROT_WRITE, MAP_SHARED, fd, base);
    if (vaddr == MAP_FAILED) {
        fatal((char *)"rpio-pwm: Failed to map peripheral at 0x%08x: %m\n", base);
        return NULL;
    }
    close(fd);
...
clear_channel(int channel)
{
    ...
    log_debug((char *)"clear_channel: channel=%d\n", channel);
    if (!channels[channel].virtbase)
        return fatal((char *)"Error: channel %d has not been initialized with 'init_channel(..)'\n", channel);
...
clear_channel_gpio(int channel, int gpio)
{
    ...
    log_debug((char *)"clear_channel_gpio: channel=%d, gpio=%d\n", channel, gpio);
    if (!channels[channel].virtbase)
        return fatal((char *)"Error: channel %d has not been initialized with 'init_channel(..)'\n", channel);
    if ((gpio_setup & 1<<gpio) == 0)
        return fatal((char *)"Error: cannot clear gpio %d; not yet been set up\n", gpio);
...
add_channel_pulse(int channel, int gpio, int width_start, int width)
{
    ...
    if (width_start + width > channels[channel].width_max || width_start < 0)
        return fatal((char *)"Error: cannot add pulse to channel %d: width_start+width exceed max_width of %d\n", channels[channel].width_max);
...
make_pagemap(int channel)
{
    ...
    if (channels[channel].page_map == 0)
        return fatal((char *)"rpio-pwm: Failed to malloc page_map: %m\n");
    memfd = open("/dev/mem", O_RDWR);
    if (memfd < 0)
        return fatal((char *)"rpio-pwm: Failed to open /dev/mem: %m\n");
    pid = getpid();
    sprintf(pagemap_fn, "/proc/%d/pagemap", pid);
    fd = open(pagemap_fn, O_RDONLY);
    if (fd < 0)
        return fatal((char *)"rpio-pwm: Failed to open %s: %m\n", pagemap_fn);
    if (lseek(fd, (uint32_t)channels[channel].virtbase >> 9, SEEK_SET) !=
                        (uint32_t)channels[channel].virtbase >> 9) {
        return fatal((char *)"rpio-pwm: Failed to seek on %s: %m\n", pagemap_fn);
    }
    for (i = 0; i < channels[channel].num_pages; i++) {
        ...
        if (read(fd, &pfn, sizeof(pfn)) != sizeof(pfn))
            return fatal((char *)"rpio-pwm: Failed to read %s: %m\n", pagemap_fn);
        if (((pfn >> 55) & 0x1bf) != 0x10c)
            return fatal((char *)"rpio-pwm: Page %d not present (pfn 0x%016llx)\n", i, pfn);
...
init_virtbase(int channel)
{
    ...
    if (channels[channel].virtbase == MAP_FAILED)
        return fatal((char *)"rpio-pwm: Failed to mmap physical pages: %m\n");
    if ((unsigned long)channels[channel].virtbase & (PAGE_SIZE-1))
        return fatal((char *)"rpio-pwm: Virtual address is not page aligned\n");
    return EXIT_SUCCESS;
}
...
{
    log_debug((char *)"Initializing channel %d...\n", channel);
    if (_is_setup == 0)
        return fatal((char *)"Error: you need to call `setup(..)` before initializing channels\n");
    if (channel > DMA_CHANNELS-1)
        return fatal((char *)"Error: maximum channel is %d (requested channel %d)\n", DMA_CHANNELS-1, channel);
    if (channels[channel].virtbase)
        return fatal((char *)"Error: channel %d already initialized.\n", channel);
    if (subcycle_time_us < SUBCYCLE_TIME_US_MIN)
        return fatal((char *)"Error: subcycle time %dus is too small (min=%dus)\n", subcycle_time_us, SUBCYCLE_TIME_US_MIN);
...
{
    if (channel > DMA_CHANNELS - 1)
        return fatal((char *)"Error: you tried to print channel %d, but max channel is %d\n", channel, DMA_CHANNELS-1);
    log_debug((char *)"Subcycle time: %dus\n", channels[channel].subcycle_time_us);
    log_debug((char *)"PW Increments: %dus\n", pulse_width_incr_us);
    log_debug((char *)"Num samples:   %d\n", channels[channel].num_samples);
    log_debug((char *)"Num CBS:       %d\n", channels[channel].num_cbs);
    log_debug((char *)"Num pages:     %d\n", channels[channel].num_pages);
    return EXIT_SUCCESS;
}
...
setup(int pw_incr_us, int hw)
{
    ...
    if (_is_setup == 1)
        return fatal((char *)"Error: setup(..) has already been called before\n");

    log_debug((char *)"Using hardware: %s\n", delay_hw == DELAY_VIA_PWM ? "PWM" : "PCM");
    log_debug((char *)"PW increments:  %dus\n", pulse_width_incr_us);
